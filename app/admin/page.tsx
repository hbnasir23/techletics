...
import { useState, useEffect } from 'react'
import { Button } from '@/components/ui/button'
import { Eye, EyeOff, LogOut, Phone, Mail, User, Hash, Calendar, Users, Filter, Search, Download, RefreshCw, Trash2, X } from 'lucide-react'
import toast, { Toaster } from 'react-hot-toast'
...
const ADMIN_CREDENTIALS = [
  { email: "enayat@", password: "e86" },
  { email: "haris@", password: "e84" },
  { email: "affan@", password: "e58" },
  { email: "anas@", password: "e87" },
]

const getSectionFromRollNo = (rollNo) => {
  if (!rollNo) return 'N/A'
  const match = rollNo.match(/-(\d{5})$/) || rollNo.match(/(\d{5})$/)
  if (!match) return 'N/A'
  const digits = match[1]
  const batch = digits.slice(0, 2)
  const serial = parseInt(digits.slice(2), 10)
  const programMap = {
    '22': 'BESE',
    '23': 'TESE',
    '24': 'SESE',
  ...
  const program = programMap[batch] ?? 'N/A'
  if (program === 'N/A' || Number.isNaN(serial)) return 'N/A'
  const sectionLetter = serial < 50 ? 'A' : 'B'
  return `${program}-${sectionLetter}`
}
...
const fetchRegistrations = async () => {
  try {
    ...
    data.forEach((reg) => {
      const teamKey = reg.team_id ? `team-${reg.team_id}-sport-${reg.sport_id}` : `player-${reg.player_id}-sport-${reg.sport_id}`

      if (!registrationsMap.has(teamKey)) {
        const sportType = reg.sport.max_team_size === 1 ? 'solo' : reg.sport.max_team_size === 2 ? 'doubles' : 'team'
        const gender = reg.gender || reg.player?.gender || 'male'
        ...
        const registration = registrationsMap.get(teamKey)
        const allowedMembers = Math.max(0, registration.sport.maxTeamSize - 1)
        if (registration.teamMembers.length >= allowedMembers) return
        registration.teamMembers.push({
          name: reg.player.name,
          rollNo: reg.player.roll_number
...
export default function AdminDashboard() {
  ...
  const [isLoading, setIsLoading] = useState(false)
  const [deleteModal, setDeleteModal] = useState({ isOpen: false, registration: null })
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch('/api/registrations/delete', {
        ...
        body: JSON.stringify({
          registrationId: deleteModal.registration.id,
          sportId: deleteModal.registration.sportId,
          teamId: deleteModal.registration.teamId
        }),
      })
      ...
      closeDeleteModal()
      loadRegistrations() // Reload the registrations
    } catch (error) {
  ...
  const openWhatsApp = (phone, name, sport) => {
    const message = `Hello ${name}! This is regarding your ${sport} registration.`
    const url = `https://wa.me/${phone}?text=${encodeURIComponent(message)}`
    window.open(url, '_blank')
  }
  ...
  const sportOptions = Array.from(new Set(registrations.map(r => r.sport.name)))
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
        ...
        <div className="w-full max-w-md bg-slate-800/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700 p-8">
          <div className="text-center mb-8">
          ...
          <div className="space-y-5">
            ...
            <Button onClick={handleLogin} className="w-full bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold py-3">
              Sign In
            </Button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4 md:p-8">
      ...
      {deleteModal.isOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl shadow-2xl border border-slate-700 max-w-md w-full p-6 relative">
            <button
            ...
            {deleteModal.registration && (
              <div className="bg-slate-900/50 rounded-lg p-4 mb-6 space-y-2">
                <p className="text-sm text-gray-300">
                  <span className="font-semibold">Sport:</span> {deleteModal.registration.sport.name}
                </p>
                <p className="text-sm text-gray-300">
                  <span className="font-semibold">Captain:</span> {deleteModal.registration.captain.captainName}
                </p>
                <p className="text-sm text-gray-300">
                  <span className="font-semibold">Roll No:</span> {deleteModal.registration.captain.captainRollNo}
                </p>
                {deleteModal.registration.teamMembers.length > 0 && (
                  <p className="text-sm text-gray-300">
                    <span className="font-semibold">Team Members:</span> {deleteModal.registration.teamMembers.length}
                  </p>
                )}
              </div>
            )}
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">Techletics 2025 Registrations</p>
        </div>
        <div className="flex gap-3">
          <Button onClick={loadRegistrations} variant="outline" className="border-gray-600 text-gray-300">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-xl p-4 shadow-lg">
          <div className="text-3xl font-bold text-white">{stats.male}</div>
          <div className="text-cyan-100 text-sm">Male</div>
        </div>
        <div className="bg-gradient-to-br from-pink-500 to-pink-600 rounded-xl p-4 shadow-lg">
          <div className="text-3xl font-bold text-white">{stats.female}</div>
          <div className="text-pink-100 text-sm">Female</div>
        </div>
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-4 shadow-lg">
          <div className="text-3xl font-bold text-white">{stats.team}</div>
          <div className="text-purple-100 text-sm">Team</div>
        </div>
        <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-xl p-4 shadow-lg">
          <div className="text-3xl font-bold text-white">{stats.doubles}</div>
          <div className="text-indigo-100 text-sm">Doubles</div>
        </div>
        <div className="bg-gradient-to-br from-violet-500 to-violet-600 rounded-xl p-4 shadow-lg">
          <div className="text-3xl font-bold text-white">{stats.solo}</div>
          <div className="text-violet-100 text-sm">Solo</div>
        </div>
      </div>

      {/* Filters */}
      <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700 p-6 mb-6">
        <div className="flex items-center gap-2 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            ...
            <select
              value={filters.sportsName}
              onChange={(e) => setFilters(prev => ({ ...prev, sportsName: e.target.value }))}
              className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-cyan-400 focus:outline-none"
            >
              <option value="all">All Sports</option>
              {sportOptions.map(sport => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Roll Number Range</label>
            <select
              value={filters.rollRange}
              onChange={(e) => setFilters(prev => ({ ...prev, rollRange: e.target.value }))}
              className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-cyan-400 focus:outline-none"
            >
              <option value="all">All</option>
              <option value="22-25">22001 – 25999</option>
      ...
      {/* Results Count */}
      <div className="mb-4">
        <p className="text-gray-400">Showing {filteredRegistrations.length} of {registrations.length} registrations</p>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700">
            <div className="text-gray-400">
              {registrations.length === 0 ? 'No registrations found in database.' : 'No registrations found matching your filters.'}
            </div>
          </div>
        ) : (
          filteredRegistrations.map((reg) => (
            <div key={reg.id} className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700 p-6 hover:border-cyan-400 transition-colors">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                <div className="flex items-start gap-3">
                  <div className="text-4xl">{reg.sport.icon}</div>
                  <div>
                    <h3 className="text-xl font-bold text-white">{reg.sport.name}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                <div className="flex items-center gap-2 text-gray-300">
                  ...
                  <span className="text-sm">Captain: {reg.captain.captainName}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-300">
                  <Hash size={16} className="text-cyan-400" />
                  <span className="text-sm">Roll: {reg.captain.captainRollNo}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-300">
                  <Mail size={16} className="text-cyan-400" />
                  <span className="text-sm">Email: {reg.captain.captainEmail}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-300">
                  <Phone size={16} className="text-cyan-400" />
                  <span className="text-sm">Phone: {reg.captain.captainPhone}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-300">
                  <Users size={16} className="text-cyan-400" />
                  <span className="text-sm">Section: {reg.captain.section}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-300">
                  <Calendar size={16} className="text-cyan-400" />
                  <span className="text-sm">Registered: {new Date(reg.registeredAt).toLocaleDateString()}</span>
                </div>
              </div>

              {reg.captain.teamName && (
                <div className="mb-4 p-3 bg-slate-900/50 rounded-lg">
                  <span className="text-sm text-gray-300">Team Name: {reg.captain.teamName}</span>
                </div>
              )}
